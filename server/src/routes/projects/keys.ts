...
router.get(
  "/keys",
  withProjectRouteError("getting API keys", "Failed to get API keys", (req, res) => {
    const keys = db
      .prepare(
        `
...
router.post(
  "/keys",
  withProjectRouteError("creating API key", "Failed to create API key", (req, res) => {
    const { label = "", expires_at } = req.body as {
      label?: unknown;
      expires_at?: unknown;
    };
...
router.delete(
  "/keys/:keyId",
  withProjectRouteError("deleting API key", "Failed to delete API key", (req, res) => {
